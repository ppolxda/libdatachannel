...
/// @param id Client ID
/// @returns Client
shared_ptr<Client> createPeerConnection(const Configuration &config, weak_ptr<WebSocket> wws,
                                        string id);
...
/// @param opusSamples Directory with opus samples
/// @returns Stream object
shared_ptr<Stream> createStreamMp4(const string mp4File, const unsigned fps,
                                   const string opusSamples);
...
const string defaultMP4File = "/home/jzd/Desktop/Project/yumaoqiu/round1_right_16_9.mp4";
string mp4File = defaultMP4File;
...
int main(int argc, char **argv) try {
	...
	int c = 0;
	auto parser =
	    ArgParser({{"a", "audio"}, {"b", "video"}, {"d", "ip"}, {"p", "port"}, {"m", "mp4"}},
	              {{"h", "help"}, {"v", "verbose"}});
	...
	if (printHelp) {
		cout << "usage: stream-h264 [-a opus_samples_folder] [-b h264_samples_folder] [-d "
		        "ip_address] [-p port] [-v] [-h]"
		     << endl
		     << "Arguments:" << endl
		     << "\t -a " << "Directory with opus samples (default: " << defaultOpusSamplesDirectory
		     << ")." << endl
		     << "\t -b " << "Directory with H264 samples (default: " << defaultH264SamplesDirectory
		     << ")." << endl
		     << "\t -d " << "Signaling server IP address (default: " << defaultIPAddress << ")."
		     << endl
	...
	string stunServer = "stun:user:user@192.168.1.22:3478?transport=udp";
	cout << "STUN server is " << stunServer << endl;
	config.iceServers.emplace_back(stunServer);
	string turnServer = "turn:user:user@192.168.1.22:3478?transport=udp";
	cout << "TURN server is " << turnServer << endl;
	...
	ws->onMessage([&](variant<binary, string> data) {
		...
		MainThread.dispatch([message, config, ws]() { wsOnMessage(message, config, ws); });
	});
...
shared_ptr<ClientTrackData> addVideo(const shared_ptr<PeerConnection> pc, const uint8_t payloadType,
	...
	auto rtpConfig =
	    make_shared<RtpPacketizationConfig>(ssrc, cname, payloadType, H264RtpPacketizer::ClockRate);
	// create packetizer
	auto packetizer =
	    make_shared<H264RtpPacketizer>(NalUnit::Separator::LongStartSequence, rtpConfig);
	// add RTCP SR handler
...
shared_ptr<ClientTrackData> addAudio(const shared_ptr<PeerConnection> pc, const uint8_t payloadType,
	...
	// create RTP configuration
	auto rtpConfig = make_shared<RtpPacketizationConfig>(ssrc, cname, payloadType,
...
shared_ptr<Client> createPeerConnection(const Configuration &config, weak_ptr<WebSocket> wws,
	...
	pc->onStateChange([id](PeerConnection::State state) {
		...
		if (state == PeerConnection::State::Disconnected ||
		    state == PeerConnection::State::Failed || state == PeerConnection::State::Closed) {
	...
	client->video =
	    addVideo(pc, 102, 1, "video-stream", "stream1", [id, wc = make_weak_ptr(client)]() {
	...
	client->audio =
	    addAudio(pc, 111, 2, "audio-stream", "stream1", [id, wc = make_weak_ptr(client)]() {
		    MainThread.dispatch([wc]() {
			    if (auto c = wc.lock()) {
				    addToStream(c, false);
			    }
...
shared_ptr<Stream> createStream(const string h264Samples, const unsigned fps,
	...
	stream->onSample([ws = make_weak_ptr(stream)](Stream::StreamSourceType type,
	                                              uint64_t sampleTime, rtc::binary sample) {
		vector<ClientTrack> tracks{};
		string streamType = type == Stream::StreamSourceType::Video ? "video" : "audio";
		// get track for given type
		function<optional<shared_ptr<ClientTrackData>>(shared_ptr<Client>)> getTrackData =
		    [type](shared_ptr<Client> client) {
		...
		if (!tracks.empty()) {
			for (auto clientTrack : tracks) {
				...
				cout << "Sending " << streamType
				     << " sample with size: " << to_string(sample.size()) << " to " << client
				     << endl;
				try {
					// send sample
...
shared_ptr<Stream> createStreamMp4(const string mp4File, const unsigned fps,
                                   const string opusSamples) {
	// video source
	auto video = make_shared<MP4FileParser>(mp4File, fps, true);
	// audio source
	auto audio = make_shared<OPUSFileParser>(opusSamples, true);
	...
	stream->onSample([ws = make_weak_ptr(stream)](Stream::StreamSourceType type,
		...
		if (!tracks.empty()) {
			for (auto clientTrack : tracks) {
				...
				cout << "Sending " << streamType
				     << " sample with size: " << to_string(sample.size()) << " sample duration: "
				     << std::chrono::duration<double, std::micro>(sampleTime).count() << " to "
				     << client << endl;
				try {
					// send sample
					trackData->track->sendFrame(
...
void startStream() {
	...
	if (avStream.has_value()) {
		...
		stream = createStreamMp4(mp4File, 30, opusSamplesDirectory);
		avStream = stream;
	}
	stream->start();
}
...
void sendInitialNalus(shared_ptr<Stream> stream, shared_ptr<ClientTrackData> video) {
	auto h264 = dynamic_cast<MP4FileParser *>(stream->video.get());
	auto initialNalus = h264->initialNALUS();

	// send previous NALU key frame so users don't have to wait to see stream works
	if (!initialNalus.empty()) {
		const double frameDuration_s = double(h264->getSampleDuration_us()) / (1000 * 1000);
		const uint32_t frameTimestampDuration =
		    video->sender->rtpConfig->secondsToTimestamp(frameDuration_s);
		video->sender->rtpConfig->timestamp =
		    video->sender->rtpConfig->startTimestamp - frameTimestampDuration * 2;
