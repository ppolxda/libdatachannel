...
Stream::Stream(std::shared_ptr<StreamSource> video)
    : std::enable_shared_from_this<Stream>(), video(video), audio(nullptr) {}

Stream::~Stream() { stop(); }

std::pair<std::shared_ptr<StreamSource>, Stream::StreamSourceType>
Stream::unsafePrepareForSample() {
	std::shared_ptr<StreamSource> ss;
	StreamSourceType sst;
	uint64_t nextTime;
	if (audio.get() != nullptr && audio->getSampleTime_us() < video->getSampleTime_us()) {
		ss = audio;
